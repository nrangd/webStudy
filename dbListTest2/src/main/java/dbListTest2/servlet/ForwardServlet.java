...
import dbListTest2.process.NotFound;
import dbListTest2.process.Process;
import dbListTest2.process.bbs.*;
...
public class ForwardServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		cmd_mapper.put("/bbs/list", new BBS_List());
		cmd_mapper.put("/bbs/add", new BBS_Add());
		cmd_mapper.put("/bbs/edit", new BBS_Edit());
		cmd_mapper.put("/bbs/delete", new BBS_Delete());
		cmd_mapper.put("/404", new NotFound());
		...
		if(service != null) {
			...
			if(nextServlet.endsWith("redirect:")) {
			...
			// 잘못된 페이지 링크로 들어왔을때 404로 리다이렉트 시켜버림
		...
		// bbs 테이블의 목록을 보여주는 페이지
			...
			request.getRequestDispatcher("/WEB-INF/views/bbs/bbs_list.jsp").forward(request, response);
		} else if(uri.equals("/bbs/add")) {
		// bbs 테이블에 내용(컬럼)을 추가해주는 페이지
			if(method.equals("GET")) {
				// 이 페이지를 링크를 통해 혹은 직접들어왔을경우
				// 테이블을 추가해주는 bbs_add.jsp로 포워드해준다
				request.getRequestDispatcher("/WEB-INF/views/bbs/bbs_add.jsp").forward(request, response);				
		...
		// 테이블 내용을 수정할 수 있다
			request.getRequestDispatcher("/WEB-INF/views/bbs/bbs_edit.jsp").forward(request, response);

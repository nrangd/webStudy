		...
		if(method.equals("GET")) {
			...
			if(uri.endsWith("s")) {
				...
				if(user_id != null && user_password != null &&
					...
					int login_try_count = 
							session.getAttribute("login_try_count") == null ?
									0 : (int)session.getAttribute("login_try_count");
					
					session.setAttribute("login_success", false);
					session.setAttribute("login_try_count", (int)login_try_count + 1);
				}
				
				response.sendRedirect("/chap03/attribute/test/loginTest.jsp");
			}
		}
	}
}
